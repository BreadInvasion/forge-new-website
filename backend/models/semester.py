...
class UserSemesterAssociation(Base):
    ...
    semester_id: Mapped[UUID] = mapped_column(DB_UUID(as_uuid=True), ForeignKey("semesters.id"), primary_key=True)

class OrgSemesterAssociation(Base):
    __tablename__ = "org_semester_associations"

    org_id: Mapped[UUID] = mapped_column(DB_UUID(as_uuid=True), ForeignKey("organizations.id"), primary_key=True)
    semester_id: Mapped[UUID] = mapped_column(DB_UUID(as_uuid=True), ForeignKey("semesters.id"), primary_key=True)
...
class Semester(Base):
    ...
    active_members: Mapped[list["User"]] = relationship(secondary="user_semester_associations")

    # Orgs who have paid to be active this semester
    active_orgs: Mapped[list["Org"]] = relationship(secondary="org_semester_associations")