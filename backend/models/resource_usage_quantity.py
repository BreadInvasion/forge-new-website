...
class ResourceUsageQuantity(Base):
    ...
    machine_usage_id: Mapped[UUID] = mapped_column(DB_UUID(as_uuid=True), ForeignKey("machine_usages.id"), primary_key=True)
    machine_usage: Mapped["MachineUsage"] = relationship(back_populates="resources_used")
    ...
    resource_id: Mapped[UUID] = mapped_column(DB_UUID(as_uuid=True), ForeignKey("resources.id"))
