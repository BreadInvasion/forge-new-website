...
async def get_usage_schema(
    ...
    if not machine or (
        machine.disabled and
        not has_permissions_any(
            session,
            current_user.id,
...
async def use_a_machine(
    ...
    current_semester = await session.scalar(select(State.active_semester).options(selectinload(State.active_semester)))
    if not current_semester and not has_permissions_any(
        session=session,
        user_id=current_user.id,
        permissions={Permissions.CAN_USE_MACHINES_BETWEEN_SEMESTERS, Permissions.IS_SUPERUSER}
    ):
        raise HTTPException(
    ...
    required_resource_slot_ids = {
        resource_slot.id for resource_slot in machine.type.resource_slots if not resource_slot.allow_empty
    }
    ...
    for resource_slot_id, usage_info in request.resource_usages.items():
        resource_slot = next(
            resource_slot for resource_slot in machine.type.resource_slots if resource_slot.id == resource_slot_id
        )
        valid_resources = resource_slot.valid_resources

        resource_used = next((resource for resource in valid_resources if resource.id == usage_info.resource_id), None)
