...
for _ in range(num_instances):
    ...
    instance["RCSid"] = f"{instance['lastName'].lower()[0:5]}{instance['firstName'][0].lower()}".replace(" ", "")
    instance["email"] = f"{instance['RCSid']}@rpi.edu"
    instance["password"] = 'password'
    instance["active"] = random.choice([True, False])
    instance["groups"] = random.choices(["admin", "user", "volunteer", "manager", "staff", "organization"], k=random.randint(1, 2))
    ...
    instance["usages"] = []
    if instance['groups'].count('volunteer') > 0 or instance['groups'].count('manager') > 0 or instance['groups'].count('staff') > 0 or instance['groups'].count('organization') > 0:
